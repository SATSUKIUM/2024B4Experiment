...
void adcSum_crystals(TString filepath = "../data/PhysicsRun/ROOT_FILES2/Run_004.dat.root"){
    // 入力ファイルを開く
        ...
        for(Int_t iBoard=0; iBoard<2; iBoard++){
            for(Int_t iCh=0; iCh<4; iCh++){
                ...
                for(Int_t iCell=0; iCell<1024; iCell++){
                    if(fPedestalTmin <= fTime[iBoard][iCh][iCell] && fTime[iBoard][iCh][iCell] <= fPedestalTmax){
                    ...
                    if(iCh == 1){
                        ...
                        for(Int_t iCell = fDiscriCell[iBoard][iCh];;iCell++){
                            if(fTime[iBoard][iCh][iCell] >= discriTime - 50.0 && fTime[iBoard][iCh][iCell] <= discriTime + 180.0){
                                adcSum_crystals_buf += fWaveform[iBoard][iCh][iCell] - pedeslta_sum;
                            }
                ...
                adcSum_crystals[iBoard][iCh] = adcSum_crystals_buf;
            }
        }
        newBranch->Fill();
        if(i % 10000 == 0){
