...
double_t gamma_factor(double theta){
...
double_t GetRate(double theta, double phi, double K){
    ...
    double A2_width = 2.0;
    double A2_hight = 2.0;
    double A2_length = 12.0;

    double r_S1_to_A1 = 25.0;
    double r_S2_to_A2 = 25.0;
    double r_Na_to_S1 = 25.0;
    double r_Na_to_S2 = 25.0; // 長さはcmで統一

    double S1_size = pow(S1_r, 2) * M_PI;
    double S2_size = pow(S2_r, 2) * M_PI;
    double A1_size = pow(A1_r, 2) * M_PI;
    

    double density_GSO = 6.71; // [g/cm^3]
    double density_NaI = 3.67; // [g/cm^3]
    double electron_GSO = 182.0; // Gd 64*2 + Si 14 + O 8*5
    double electron_NaI = 64.0; // Na 11 + I 53
    double molweight_GSO = 422.58;// Gd 157.25*2 + Si 28.08 + O 16*5 [g/mol]
    double molweight_NaI = 149.9; // Na 23 + I 126.9 [g/mol]
    
    double avogadro = 6.02e23; // [/mol]
    double r_e = 2.8e-13; // [cm]

    double intensity = 2.0e6; // [/s]
    double time = 60*60*24*14; // [s]
...
void PlotRateIntegral(double days = 1.0, double K = 0.78){
    ...
    double theta_min_rad = atan(r_S2_to_A2 / A2_length); // およそ64度のラジアン
    double phi_range_rad = 2 * atan((A2_width / 2) / r_S2_to_A2); //　およそ4.6度のラジアン

    double l_theta_d = (M_PI / 2 - theta_min_rad) / delta_theta;
    double l_phi_d = phi_range_rad / delta_phi;

    int l_theta = static_cast<int>(l_theta_d);
    int l_phi = static_cast<int>(l_phi_d); // 角度の範囲内にある0.1度刻みの点の個数

    int abs_points = 5; // 吸収体の個数
    // int abs_points = 181;

    double x[l_theta], y[l_phi], count_sum[abs_points], y_center[abs_points], count_error[abs_points];

    double x_min = theta_min_rad * 180 / M_PI; // 度
    double x_max = 90; // 度
    double y_min; // 度
    double y_max; // 度
    ...
    for(int i = 0; i < abs_points; i++){
        ...
        for(int m = 0; m < l_theta; m++){

            x[m] = x_min + m * delta_theta * 180 / M_PI;

            for(int n = 0; n < l_phi; n++){
